...
def extract_text_from_last_image(directory):
    ...
    # Sort image files by modification time (last modified first)
    image_files.sort(key=lambda x: os.path.getmtime(os.path.join(directory, x)), reverse=True)
    
    if image_files:
        # Load the last image file
        last_image_path = os.path.join(directory, image_files[0])
...
# Example usage:
directory_path = "/home/idc/Desktop/RaspberryPI_main_project/to read"
text = extract_text_from_last_image(directory_path)
print("Text extracted from the last image:", text)
